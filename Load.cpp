...
static long long safe_stoll(const std::string &s,
                            long long default_val) noexcept {
  try {
    std::size_t pos = 0;
...
static void kill_process_on_port(int port) {
  ...
  // Also kill by binary name pattern (catches processes in startup phase)
  std::string pattern = std::to_string(port);
  std::string kill_by_name =
      "PID=$(pgrep -x \"" + pattern +
      "\" 2>/dev/null || pgrep -f \"CONTAINER/" + pattern +
      "\" 2>/dev/null); [ -n \"$PID\" ] && kill -9 $PID 2>/dev/null; true";
...
static fs::path get_container_dir() {
  const char *env_dir = std::getenv("CONTAINER_DIR");
  if (env_dir) {
    fs::path p(env_dir);
    if (fs::exists(p)) {
...
static void start_backend(Backend *b) {
  ...
  int attempts = b->restart_attempts.fetch_add(1) + 1;
  const char *max_retries_env = std::getenv("MAX_RESTART_ATTEMPTS");
  int max_retries = max_retries_env ? safe_stoi(max_retries_env, 3) : 3;

  log_info("[LB] Launching container on port " + std::to_string(b->port) +
           " (Attempt " + std::to_string(attempts) + " of " +
           std::to_string(max_retries) + ")...");

  if (!fs::exists(exec_path)) {
    log_error("[LB] Error: Backend binary for port " + std::to_string(b->port) +
              " not found at: " + exec_path + ". Cannot launch.");
    b->failed.store(true);
    if (b->restart_attempts.load() >= max_retries) {
      log_error("[LB] Backend " + b->endpoint() +
                " has reached the maximum restart limit (" +
                std::to_string(max_retries) +
  ...
  // Launch the backend binary as a background process.
  // We deliberately use the same approach on macOS and Linux:
  // • Run the binary directly (not via osascript) so it inherits the
  //   correct working directory and environment from the load balancer.
  // • Redirect stdout/stderr to a per-port log file next to the binary
  //   so you can `tail -f` it for debugging.
  // • The trailing & detaches the child; the LB does NOT need its PID
  //   because health-check polling drives liveness tracking.
  {
    // Run from the directory that contains the binary so that any
    // relative paths inside the backend (DB config, log dirs, …) resolve
    // the same way as when you launch it manually from that folder.
    fs::path container_dir = get_container_dir();
    std::string log_file =
        (container_dir / ("container_" + std::to_string(b->port) + ".log"))
            .string();
    std::string cmd = "cd \"" + container_dir.string() + "\" && " + "\"" +
                      exec_path + "\" > \"" + log_file + "\" 2>&1 &";
    std::system(cmd.c_str());
    log_info("[LB] Backend on port " + std::to_string(b->port) +
...
static void health_checker_loop(int interval_seconds) {
  ...
  if (!g_docker_mode.load()) {
    ...
    int target_active = target_active_env ? safe_stoi(target_active_env, 3) : 3;
    int started_count = 0;
    for (auto &b : g_backends) {
  ...
  while (g_running.load()) {
    ...
    if (!g_docker_mode.load() && g_running.load()) {
      ...
      long long startup_timeout_ms =
          startup_timeout_env ? safe_stoll(startup_timeout_env, 10) * 1000LL
                              : 10000LL;
        ...
        if (b->started.load() && !b->failed.load()) {
          if (b->alive.load()) {
            ...
            if (has_failed) {
              ...
              const char *max_retries_env = std::getenv("MAX_RESTART_ATTEMPTS");
              int max_retries =
                  max_retries_env ? safe_stoi(max_retries_env, 3) : 3;
              if (b->restart_attempts.load() >= max_retries) {
                log_error("[LB] Backend " + b->endpoint() +
                          " has reached the maximum restart limit (" +
                          std::to_string(max_retries) +
      ...
      const char *target_active_env = std::getenv("TARGET_ACTIVE_BACKENDS");
      int target_active =
          target_active_env ? safe_stoi(target_active_env, 3) : 3;
...
static void on_signal(int /*sig*/) {
  ...
  if (auto *ps = g_proxy_server.load())
    ps->stop();
  if (auto *as = g_admin_server.load())
    as->stop();
}
...
int main() {
  ...
  try {
    ...
    int lb_admin_port =
        lb_admin_port_env ? safe_stoi(lb_admin_port_env, 5650) : 5650;
    ...
    if (backends_env) {
      ...
      while (std::getline(ss, item, ',')) {
        ...
        if (colon != std::string::npos && colon + 1 < item.size()) {
          ...
          if (port_val <= 0 || port_val > 65535) {
            log_error("Invalid backend port in specification (out of range): " +
                      item);
            continue;
          }
      ...
      if (backend_ports_env) {
        ...
        while (std::getline(ss, item, ',')) {
          ...
          if (port_val <= 0 || port_val > 65535) {
            log_error("Invalid/out-of-range backend port in environment: " +
                      item);
            continue;
          }
    ...
    auto proxy_handler = [](const httplib::Request &req,
      ...
      for (const auto &h : req.headers) {
        if (h.first == "Host" || h.first == "Content-Length" ||
            h.first == "Transfer-Encoding" || h.first == "Connection") {
          continue;
        }
      ...
      if (result) {
        ...
        long long old_avg =
            backend->avg_response_ms.load(std::memory_order_relaxed);
        long long new_avg =
